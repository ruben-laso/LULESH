...
PrintCommandLineOptions(char* execname, int myRank)
{
    if (myRank == 0) {
        ...
        printf(" -b <balance>    : Load balance between regions of a domain "
               "(def: 1)\n");
        printf(
            " -c <cost>       : Extra cost of more expensive regions (def: 1)\n");
        printf(" -f <numfiles>   : Number of files to split viz dump into "
               "(def: (np+10)/9)\n");
    ...
    if (argc > 1) {
        ...
        while (i < argc) {
            ...
            if (strcmp(argv[i], "-i") == 0) {
                ...
                if (!ok) {
                    ParseError("Parse Error on option -i integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
            }
            /* -s <size, sidelength> */
                ...
                if (!ok) {
                    ParseError("Parse Error on option -s integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
            ...
            /* -r <numregions> */
                ...
                if (!ok) {
                    ParseError("Parse Error on option -r integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
            ...
            /* -f <numfilepieces> */
                ...
                if (!ok) {
                    ParseError("Parse Error on option -f integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
            ...
            /* -q */
                ...
                if (!ok) {
                    ParseError("Parse Error on option -b integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
                ...
                if (!ok) {
                    ParseError("Parse Error on option -c integer value "
                               "required after argument\n",
                        myRank);
                }
                i += 2;
            ...
            /* -h */
                ...
                PrintCommandLineOptions(argv[0], myRank);
                sprintf(
                    msg, "ERROR: Unknown command line argument: %s\n", argv[i]);
...
void VerifyAndWriteFinalOutput(Real_t elapsed_time,
    ...
    Int_t numRanks)
{
    // GrindTime1 only takes a single domain into account, and is thus a good
    // way to measure processor speed indepdendent of MPI parallelism.
    ...
    std::cout << "\nElapsed time         = " << std::setw(10) << elapsed_time
              << " (s)\n";
    std::cout << std::setprecision(8);
    std::cout << "Grind time (us/z/c)  = " << std::setw(10) << grindTime1
              << " (per dom)  (" << std::setw(10) << elapsed_time
              << " overall)\n";
    std::cout << "FOM                  = " << std::setw(10)
              << 1000.0 / grindTime2 << " (z/s)\n\n";
